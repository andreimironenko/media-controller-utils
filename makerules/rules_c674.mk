...
ifeq ($(PROFILE_$(CORE)), debug)
 ...
 ifndef MODULE_NAME
 ifeq ($(FORMAT),ELF)
  CFLAGS_XDCINTERNAL += -Dxdc_cfg__header__='$(CONFIGURO_DIR)/package/cfg/DspAppMain_pe674.h' 
 else
  CFLAGS_XDCINTERNAL += -Dxdc_cfg__header__='$(CONFIGURO_DIR)/package/cfg/DspAppMain_p674.h'
 endif 
 endif
...
ifeq ($(PROFILE_$(CORE)), release)
 ...
 #LNKFLAGS_INTERNAL_PROFILE = --opt='--endian=$(ENDIAN) --abi=$(CSWITCH_FORMAT) -qq -pdsw225 $(CFLAGS_GLOBAL_$(CORE)) -oe --symdebug:dwarf -ms -op2 -O3 -k -os --optimize_with_debug --inline_recursion_limit=20' --strict_compatibility=on
# LNKFLAGS_INTERNAL_PROFILE = 
endif
...
$(OBJ_PATHS): $(OBJDIR)/%.$(OBJEXT): %.c
	$(ECHO) CL6X-CC $<
	$(Q)$(CC) -ppd=$(DEPFILE).P $(_CFLAGS) $(INCLUDES) $(CFLAGS_DIROPTS) -fc $<
	$(Q)$(CC) $(_CFLAGS) $(INCLUDES) $(CFLAGS_DIROPTS) -fc $<
...
LNKFLAGS_INTERNAL_COMMON = --warn_sections -q -e=_c_int00 --silicon_version=6740 -c  --dynamic
...
#CONFIGURO_CMD = $(xdc_PATH)/xs xdc.tools.configuro --generationOnly -o $(CONFIGURO_DIR) -t $(TARGET_XDC) -p $(PLATFORM_XDC) \
#               --cfgArgs $(CFGARGS_XDC) -r $(PROFILE_$(CORE)) -c $(CODEGEN_PATH_DSPELF) -b $(CONFIG_BLD_XDC_$(ISA)) $(XDC_CFG_FILE_NAME)               
#CONFIGURO_CMD = $(xdc_PATH)/xs xdc.tools.configuro --generationOnly -o $(CONFIGURO_DIR) -t $(TARGET_XDC) -p $(PLATFORM_XDC) \
